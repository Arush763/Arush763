...
// Using gradient placeholder colors instead of real images
const photos = [
...
function GalleryCard({
  ...
  return (
    <motion.div
      ...
      className="gallery-item rounded-xl cursor-pointer relative group"
      style={{
        gridRow: photo.size === 'large' ? 'span 2' : 'span 1',
      }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      {/* Gradient placeholder representing a car photo */}
      <div
        className={`w-full h-full min-h-[200px] bg-gradient-to-br ${photo.bg} relative overflow-hidden rounded-xl`}
      >
        ...
        <motion.div
          ...
          transition={{ duration: 0.2 }}
          className="absolute top-4 right-4 bg-white/20 backdrop-blur-sm rounded-lg p-2"
        >
          <ZoomIn className="w-4 h-4 text-white" />
        </motion.div>

        {/* Animated spinning wheel on hover */}
        <motion.div
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-gray-500 max-w-xl mx-auto">
            Every car we touch is a testament to our craft. See the transformations for yourself.
          </p>
        </motion.div>
        ...
        >
          <p className="text-gray-500 text-sm">
            ...
            <a href="/booking" className="text-red-400 hover:text-red-300 font-medium">
              Book a service →
            </a>
          </p>
        </motion.div>
