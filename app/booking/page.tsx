    ...
    <div className="min-h-screen bg-[#0a0a0a] pt-28 pb-24">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl sm:text-5xl font-black text-white mt-3 mb-4">Reserve Your <span className="text-gradient">Appointment</span></h1>
          <p className="text-gray-500 max-w-md mx-auto">Fill out the form below. We'll confirm within 24 hours.</p>
        </div>
          ...
          <div className="space-y-4">
            <div className="bg-[#0e0e0e] border border-white/5 rounded-xl p-6">
              ...
              <ul className="space-y-3">
                {[{icon: Clock, text: 'Takes under 2 minutes'},{icon: Shield, text: 'No obligation estimate'},{icon: Car, text: 'Any service, any vehicle'}].map((item) => {
                  const Icon = item.icon
                  return (<li key={item.text} className="flex items-center gap-3 text-gray-400 text-sm"><Icon className="w-4 h-4 text-red-500 shrink-0" />{item.text}</li>)
                })}
              </ul>
            </div>
            ...
            <div className="bg-[#0e0e0e] border border-white/5 rounded-xl p-6">
              ...
              <a href="tel:+15555555555" className="text-red-400 hover:text-red-300 text-xl font-bold transition-colors">(555) 555-5555</a>
            </div>
          </div>
        </div>
      </div>
